...
describe('MerkleTreeWithHistoryCustom', function () {
  this.timeout(20000)
  ...
  async function fixture() {
    ...
    return { merkleTreeWithHistory, token, tornadoPool, omniBridge }
  }

  describe('CustomTest', () => {
    it('Estimate gas, deposit in L1, withdraw in L2', async () => {
      const { merkleTreeWithHistory, token, tornadoPool, omniBridge } = await loadFixture(fixture)

      // estimate and print gas needed to insert a pair of leaves to
      ...
      const aliceChangeAmount = aliceDepositAmount.sub(aliceWithdrawAmount)
      ...
      expect(tornadoPoolBalance).to.be.equal(aliceChangeAmount)
    })
  })
})
